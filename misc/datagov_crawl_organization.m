...
save organizations2 organizations_base_url organizations
%%
clear dados
...
if ~exist('organizations','var')
    load organizations organizations
end
...
for d = 1 : length(dados)
    ...
    furl = @(p)[url_base dados(d).href(1:8) p dados(d).href(9:end)];
    
    clipboard('copy',furl('/gov'))
    ...
    for i = 1 : length(data_info)
        %%
       info_field = char(data_info(i).findElement('th').extractHTMLText);
...
%        info_date = char(data_info(d).findElement('span').getAttribute('data-datetime'));
       ...
       info_value = strtrim(char(data_info(i).findElement('td').extractHTMLText));
       
%        fprintf('%s: %s (%s)\n', info_field, info_value, info_date)
       
       info.(info_field) = info_value;
    end
